#!/bin/bash
# Claude Code hook script for the Raycast "Del2" extension.
#
...
#   - auto_name: updated on every prompt to the latest user prompt (truncated to 50 chars)
...
#     over auto_name in the Raycast UI.
...
# preserving all other fields (auto_name, custom_name, cwd, pid, etc.).
...
case "$HOOK_EVENT" in
    ...
    "UserPromptSubmit")
        ...
        if [[ "$USER_PROMPT" =~ ^[Nn][Aa][Mm][Ee]:[[:space:]]*(.*) ]]; then
            ...
            if [[ -f "$STATE_FILE" ]]; then
                jq --arg name "$CUSTOM_NAME" '.custom_name = $name' "$STATE_FILE" > "${STATE_FILE}.tmp" && mv "${STATE_FILE}.tmp" "$STATE_FILE"
            fi
        ...
        AUTO_NAME=$(echo "$USER_PROMPT" | cut -c1-25)
        update_status "working"
        if [[ -n "$AUTO_NAME" && -f "$STATE_FILE" ]]; then
            jq --arg name "$AUTO_NAME" '.auto_name = $name' "$STATE_FILE" > "${STATE_FILE}.tmp" && mv "${STATE_FILE}.tmp" "$STATE_FILE"
        fi
        ;;
    "PreToolUse")
